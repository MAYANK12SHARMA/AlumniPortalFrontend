    ...
    async function load() {
      try {
        const [s, r] = await Promise.allSettled([
          getAdminDashboardStats().catch(() => ({ stats: {} })),
          getRoleRequests().catch(() => ({ results: [], items: [] })),
        ]);
        if (!mounted) return;
        const rawStats = s.status === "fulfilled" ? s.value : { stats: {} };
        const mapped = mapStats(rawStats);
        setStats(mapped);
        const list =
          r.status === "fulfilled"
            ? r.value.results || r.value.items || []
            : [];
          ...
          {loading ? (
            ...
            <>
              ...
              <StatCard
                ...
                value={stats?.role_requests_accepted ?? 0}
                delta="+0.6%"
            ...
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <Briefcase size={16} className="text-yellow-400" /> Example
                  Jobs (Coming Soon)
                </CardTitle>
              </CardHeader>
              <CardContent>
