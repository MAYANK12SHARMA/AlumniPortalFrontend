...
// Local date formatting helper (short, readable)
...
export const StudentDetailView = ({ profile }: { profile: any }) => {
  ...
  const avatar =
    !imgErr && profile.profile_picture
      ...
      : "";
  const headerMeta = [
    profile.program && String(profile.program).toUpperCase(),
  ...
  // Social Platforms (future extensibility) – union type only (no runtime array needed)
  type SocialKey =
    | "linkedin"
    | "twitter"
    | "instagram"
  ...
  // Map possible field names on Student profile (back‑end TBD). Using existing linkedin_url & generic fallbacks.
  const socialData: Array<{
    ...
    if (profile.social_links && typeof profile.social_links === "object") {
      Object.entries(profile.social_links).forEach(([k, v]: any) => {
        if (!v) return;
        const url = v.url || v.link || v.href;
        const label = v.username || v.handle || k;
        if (!url) return;
        const iconMap: Record<string, React.ReactNode> = {
          github: <LinkIcon className="h-4 w-4" />,
        ...
        add(
          ...
          label,
          url,
          iconMap[k] || <LinkIcon className="h-4 w-4" />,
          k === "github" ? "violet" : "emerald"
        );
      });
    }
    add(
    ...
    if (profile.is_public && (profile.phone || profile.phone_number)) {
      const raw = String(profile.phone || profile.phone_number).replace(
        /[^\d+]/g,
        ""
      );
      const wa = `https://wa.me/${raw.replace(/^\+/, "")}`;
      add(
        "whatsapp",
        "WhatsApp",
        wa,
    ...
    <div className="relative">
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-6 pt-6 pb-24 space-y-10">
        <div className="flex items-center gap-4">
          <Link
            href="/dashboard/directory/students"
        ...
        >
          ...
          <div className="flex flex-col lg:flex-row gap-10">
            ...
            <div className="flex-1 space-y-8">
              ...
              {(profile.technical_skills?.length ||
                ...
                <section className="space-y-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(profile.technical_skills || []).map((s: string) => (
                      <span
                        key={"tech-" + s}
                    ...
                    {(profile.soft_skills || []).map((s: string) => (
                      <span
                        key={"soft-" + s}
              ...
              {(profile.major_coursework?.length ||
                ...
                <section className="grid gap-6 md:grid-cols-2">
                  ...
                  {profile.extracurricular_activities ? (
                    <CardList
                      ...
                      items={[profile.extracurricular_activities]}
                      icon={<Sparkles className="h-4 w-4" />}
                    />
                  ) : null}
                </section>
...
// Skeleton while loading profile detail
...
const CardList = ({
  ...
  <motion.div
    ...
    transition={{ duration: 0.4 }}
    viewport={{ once: true, amount: 0.4 }}
    className="relative overflow-hidden rounded-xl border border-zinc-800/70 bg-zinc-900/40 p-4"
  >
    <div className="flex items-start gap-3 mb-3">
      <div className="mt-0.5 flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-amber-500/20 to-orange-500/10 border border-amber-400/20 text-amber-300">
        {icon}
      </div>
      <h3 className="text-[11px] uppercase tracking-wide text-zinc-500 font-medium">
        {title}
      </h3>
    </div>
    <ul className="flex flex-wrap gap-2">
      {items.map((i) => (
        <li
          key={i}
          className="px-2 py-1 rounded-full text-[10px] font-medium bg-zinc-800/60 border border-zinc-700/60 text-zinc-300"
        >
          {i}
        </li>
      ))}
    </ul>
    <div className="pointer-events-none absolute inset-0 opacity-0 hover:opacity-100 transition-opacity duration-500 bg-[radial-gradient(circle_at_85%_15%,rgba(251,191,36,0.12),transparent_65%)]" />
  </motion.div>
);

// Reusable dialog for Student contact / social links
const ContactDialog = ({
  open,
  onClose,
  socials,
